...
manage_wifi() {
    ...
    if [ -z "$chosen_network" ]; then
        ...
        local device_status=$(nmcli -t -f GENERAL device show wlp1s0 | grep STATE | cut -d: -f2)
        ...
        case $action in
            "󰸋  Connect")
                ...
                ;;
            "  Disconnect")
                nmcli device disconnect wlp1s0 && notify-send "Disconnected" "You have been disconnected from $chosen_id."
                ;;
            "  Forget")
